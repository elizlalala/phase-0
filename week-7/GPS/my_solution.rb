...
#Release 3:
#STATE_DATA is a constant variable that has a scope of the entire program.
STATE_DATA = {
...
#Release 2:
# require_relative: Require "relative" refers to a neighboring file within the same folder as the "calling" file.  Require can call a file from any path, stored in a different folder/drive/etc.

class VirusPredictor

  #Initializes an instance of class VirusPredictor and sets the 3 necessary arguments to instance variables.
  ...
  #Virus is calling the 2 methods: predicted deaths and speed of spread in order to calculate and print the anticipated effects of the virus per a specific state.
  #If the method definition doesn't require arguments, then why do the contained methods require arguments?
  #Release 6: deleted arguments
  def virus_effects
    predicted_deaths
    speed_of_spread
  end
  ...
  #Predicted Deaths uses flow control logic to apply a established rubric of rates of death to states with varying population densities.  It requires 3 arguments: pop density, pop, and the state as inputs.  It prints the predicted rate of death as output.
  ...
  #Speed of Spread uses the inputs pop density and the state name to print out the amount of time in which the set of rates of death are applicable.
...
#Release 5:
#50 states report to replace the above
STATE_DATA.each {|state, hash_values|
    state = VirusPredictor.new(state, hash_values[:population_density], hash_values[:population])
    state.virus_effects
  }
